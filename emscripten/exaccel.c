...
int main() {
  ...
  if ((!page[0]) || (!page[1]) || (!vimage)) {
    set_gfx_mode(GFX_TEXT, 0, 0, 0, 0);
    allegro_message(
        "Not enough video memory (need two 1024x768 pages "
        "and a 320x200 image)\n");
    return 1;
  }
  ...
  while (!done) {
    ...
    for (i = 0; i < num_images; i++)
      blit(vimage, page[page_num], 0, 0, images[i].x, images[i].y, vimage->w,
           vimage->h);
    ...
    if (gfx_capabilities & GFX_HW_FILL)
      textout_ex(page[page_num], font, "Clear: hardware accelerated", 0, 16,
                 255, -1);
    else
      textout_ex(page[page_num], font,
                 "Clear: software (urgh, this "
                 "is not good!)",
                 0, 16, 255, -1);

    if (gfx_capabilities & GFX_HW_VRAM_BLIT)
      textout_ex(page[page_num], font, "Blit: hardware accelerated", 0, 32, 255,
                 -1);
    else
      textout_ex(page[page_num], font,
                 "Blit: software (urgh, this program "
